...
int WINAPI WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpCmdLine,int nCmdShow)
{
	...
	if(!g_engine->Init(g_engine->GetScreenWidth(),g_engine->GetScreenHeight(),
		g_engine->GetColourDepth(),g_engine->GetFullScreen(),g_engine->GetNumberControllers()))
	{
	...
	currentTime = updateTimer.GetTimer();

	while(!gameOver)
	{
		
		DWORD newTime   = updateTimer.GetTimer();
		DWORD frameTime = newTime - currentTime;
		currentTime     = updateTimer.GetTimer();
		...
		g_engine->Update(frameTime/1000.0f);
	}
